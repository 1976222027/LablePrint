        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case READ_DATA:
                    ...
                    String status = LabelPrinter.getContexts().getString(R.string.str_printer_conn_normal);
                    if (sendCommand == esc) {
                        //设置当前打印机模式为ESC模式
                        if (currentPrinterCommand == null) {
                            ...
                            if (result == 0) {//打印机状态查询
                                ...
                                LabelPrinter.getContexts().sendBroadcast(intent);
                            } else if (result == 1) {//查询打印机实时状态
                                if ((buffer[0] & ESC_STATE_PAPER_ERR) > 0) {
                                    status += " "+ LabelPrinter.getContexts().getString(R.string.str_printer_out_of_paper);
                                }
                                if ((buffer[0] & ESC_STATE_COVER_OPEN) > 0) {
                                    status += " "+ LabelPrinter.getContexts().getString(R.string.str_printer_open_cover);
                                }
                                if ((buffer[0] & ESC_STATE_ERR_OCCURS) > 0) {
                                    status += " "+ LabelPrinter.getContexts().getString(R.string.str_printer_error);
                                }
                                System.out.println(LabelPrinter.getContexts().getString(R.string.str_state) + status);
                                String mode= LabelPrinter.getContexts().getString(R.string.str_printer_printmode_esc);
                                Utils.toast(LabelPrinter.getContexts(), mode+" "+status);
                            }
                        }
                        ...
                        if (currentPrinterCommand == null) {
                            ...
                            if (cnt == 1) {//查询打印机实时状态
                                ...
                                System.out.println(LabelPrinter.getContexts().getString(R.string.str_state) + status);
                                String mode= LabelPrinter.getContexts().getString(R.string.str_printer_printmode_tsc);
                                Utils.toast(LabelPrinter.getContexts(), mode+" "+status);
                            } else {//打印机状态查询
                                Intent intent = new Intent(ACTION_QUERY_PRINTER_STATE);
                                intent.putExtra(DEVICE_ID, id);
                        ...
                        if (currentPrinterCommand == null) {
                            ...
                            if (cnt == 1) {
                                ...
                                String mode= LabelPrinter.getContexts().getString(R.string.str_printer_printmode_cpcl);
                                Utils.toast(LabelPrinter.getContexts(), mode+" "+status);
                            } else {//打印机状态查询
                                Intent intent = new Intent(ACTION_QUERY_PRINTER_STATE);
                                intent.putExtra(DEVICE_ID, id);

...
public class LabelCommand {
 ...
 public void addText(int x, int y, LabelCommand.FONTTYPE font, LabelCommand.ROTATION rotation, double Xscal, double Yscal, String text) {
        new String();
        String str = "TEXT " + x + "," + y + "," + "\"" + font.getValue() + "\"" + "," + rotation.getValue() + "," + Xscal + "," + Yscal + "," + "\"" + text + "\"" + "\r\n";
        this.addStrToCommand(str);
    }
...
// 注意的是,如果是条码内容位数是“奇数”的纯数字,
// 那么就需要把条码内容给拆分,前面偶数用Code128 C码,后面一位奇数用B码或者A码。
/**
 * 类型选择Code128时,会根据条形码内容,自动选择A码、B码、C码或者组合码的形式,以最短的方式编码图形*/
    // 1英寸(in)=25.4毫米(mm)//条形码密度为16.39mil  条码宽 L=（11*字符数+35）*0.01639=x英寸=x*25.4毫米
    //如果 采用code128C采用2位解码  的纯数字(2位一字符)  L = (5.5字符数 + 35)*0.416306 毫米 //=0.01639*25.4
